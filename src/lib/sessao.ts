...
//
// Formato do cookie: `<sessaoId>.<expiraEm>.<papel>.<pf>.<pe>.<assinatura>`
// (`pf`/`pe` = podeFinanceiro/podeExcluir, "1" ou "0")
//
...
// operador em rota de dono, de financeiro ou de exclusão — sem precisar consultar o
// banco. A checagem que vale (sessão existe, usuário continua ativo, papel e
// permissões atuais) acontece depois, no servidor, em `auth.ts`.
//
// Papel e permissões viajam assinados, então não dá para editar o cookie e virar
// admin. Se qualquer um deles mudar no banco, as sessões daquele usuário são apagadas
// na hora (ver a rota de usuários), e o cookie antigo morre junto.
...
export async function assinarToken(
  ...
  const corpo = `${sessaoId}.${expiraEm.getTime()}.${papel}.${podeFinanceiro ? "1" : "0"}.${podeExcluir ? "1" : "0"}`;
...
export async function lerToken(token: string | undefined | null): Promise<TokenLido | null> {
  ...
  const [sessaoId, expiraTexto, papel, pf, pe, assinatura] = partes;
  ...
  try {
    const corpo = `${sessaoId}.${expiraEm}.${papel}.${pf}.${pe}`;
    ...
    return valida
      ? { sessaoId, expiraEm, papel, podeFinanceiro: pf === "1", podeExcluir: pe === "1" }
      : null;
  } catch {
    return null;
  }
