    ...
    <div className="px-4 sm:px-6 py-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="leading-tight">
          <h1 className="text-xl font-bold text-tinta">{isEdit ? "Editar orçamento" : "Novo orçamento"}</h1>
                  </div>
      </div>
        ...
        <div className="bg-superficie rounded-xl border border-linha p-5 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="block text-sm font-medium text-tinta-2">Cliente</label>
              <button
                type="button"
                onClick={abrirNovoCliente}
                className="text-xs font-medium text-perigo hover:text-perigo"
              >
                + Novo cliente
              </button>
            </div>
          ...
          {!clienteId && (
            <div className="rounded-lg border border-dashed border-linha-forte bg-superficie-2 p-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-tinta-3 mb-1">Nome ou referência *</label>
                  <input
                    value={clienteNome}
                ...
                <div>
                  <label className="block text-xs text-tinta-3 mb-1">Telefone</label>
                  <input
          ...
          <div>
            ...
            <select value={veiculoId} onChange={(e) => setVeiculoId(e.target.value)} disabled={!clienteId || veiculos.length === 0} className={inputCls + " disabled:bg-superficie-2 disabled:text-tinta-3"}>
            ...
            <p className="text-xs text-tinta-3 mt-1">Opcional — mas necessário para converter o orçamento em OS.</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <textarea value={descricao} onChange={(e) => setDescricao(e.target.value)} rows={3} placeholder="Ex: Revisão geral, troca de óleo e filtros..." className="w-full rounded-lg border border-linha-forte px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-y" />
              <p className="text-xs text-tinta-3 mt-1">Opcional se você já adicionar os itens abaixo.</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-tinta-2 mb-1">Validade</label>
              <input type="date" value={validade} onChange={(e) => setValidade(e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium text-tinta-2 mb-1">Observações</label>
      ...
      {showNovoCliente && (
        <Modal
          titulo="Novo cliente"
          descricao="O mínimo para o orçamento deixar de ser rascunho."
        ...
        >
          <form id="form-novo-cliente" onSubmit={salvarNovoCliente} className="space-y-3">
            <div>
              <label className="mb-1 block text-sm font-medium text-tinta-2" htmlFor="nc-nome">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1 block text-sm font-medium text-tinta-2" htmlFor="nc-tel">
                  Telefone
                </label>
                <Entrada
                  id="nc-tel"
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-tinta-2" htmlFor="nc-apelido">
                  Apelido
                </label>
                <Entrada
                  id="nc-apelido"
            ...
            <div className="space-y-3 rounded-lg bg-superficie-2 p-3">
              <p className="text-xs font-medium text-tinta-2">
                Veículo (opcional — necessário para converter em OS)
              </p>
              {veiculoDesc.trim() && (
                <p className="text-xs text-tinta-3">Anotado no rascunho: “{veiculoDesc.trim()}”</p>
              )}
              <VeiculoCampos value={novoVeiculo} onChange={setNovoVeiculo} />
            </div>

            {clienteErro && (
              <p className="rounded-lg bg-perigo-fraco px-3 py-2 text-sm text-perigo">{clienteErro}</p>
            )}
          </form>
        </Modal>
      )}
