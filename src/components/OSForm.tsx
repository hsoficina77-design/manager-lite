    ...
    <div className="px-4 sm:px-6 py-6 max-w-5xl mx-auto">
      ...
      <form onSubmit={submit}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
                  ...
                  <tr key={idx} className={editingIdx === idx ? "bg-amber-50" : "group"}>
                    ...
                    <td className="py-1 pl-2">
                      ...
                      <div className="flex items-center justify-end gap-2 sm:gap-1.5">
                        <button type="button" onClick={() => startEdit(idx)} aria-label={`Editar ${item.descricao}`} title="Editar" className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700 sm:h-8 sm:w-8">
                        ...
                        <button type="button" onClick={() => removeItem(idx)} aria-label={`Remover ${item.descricao}`} title="Remover" className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-red-400 hover:bg-red-50 hover:text-red-600 sm:h-8 sm:w-8">
          ...
          {removido && (
            <div role="status" aria-live="polite" className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
              ...
              <div className="flex shrink-0 items-center gap-1">
                <button type="button" onClick={desfazerRemocao} className="inline-flex items-center gap-1.5 rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-bold text-white hover:bg-amber-600">
                ...
                <button type="button" onClick={() => setRemovido(null)} aria-label="Dispensar aviso" title="Dispensar" className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-amber-600 hover:bg-amber-100">✕</button>
              </div>
            </div>
          )}
