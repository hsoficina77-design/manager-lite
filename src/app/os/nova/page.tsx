...
function NovaOSForm() {
  ...
  const preVeiculoId = searchParams.get("veiculoId") || "";
  ...
  const [combustivelEmUso, setCombustivelEmUso] = useState("");
  ...
  useEffect(() => {
    ...
    fetch("/api/mecanicos").then((r) => r.json()).then(setMecanicos);
  }, []);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const mecanicoFinal = mecanicoInput.trim() || mecanico;
      const payload = {
        clienteId, veiculoId, descricao,
        kmEntrada: kmEntrada || undefined,
        obs: obs || undefined,
        mecanico: mecanicoFinal || undefined,
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="mb-6">
        <Link href="/os" className="text-sm text-zinc-500 hover:text-zinc-700">← Voltar</Link>
        <h1 className="text-2xl font-bold text-zinc-900 mt-2">Nova OS</h1>
      </div>
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-zinc-200 p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <textarea value={descricao} onChange={(e) => setDescricao(e.target.value)} required rows={3} placeholder="Ex: Revisão geral, troca de óleo e filtros..." className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 resize-none" />
            </div>
            <div>
        ...
        <div className="bg-white rounded-xl border border-zinc-200 p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {mecanicos.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-zinc-700 mb-1">Selecionar existente</label>
                <select value={mecanico} onChange={(e) => { setMecanico(e.target.value); setMecanicoInput(""); }} className={inputCls}>
                  <option value="">—</option>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">
                {mecanicos.length > 0 ? "Ou digitar novo" : "Nome do mecânico"}
              </label>
              <input value={mecanicoInput} onChange={(e) => { setMecanicoInput(e.target.value); setMecanico(""); }} placeholder="Ex: João Silva" className={inputCls} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-zinc-200 p-5 space-y-4">
          ...
          <div className="grid grid-cols-12 gap-2 items-end">
            ...
            <div className="col-span-2">
              <button type="button" onClick={addItem} className="w-full rounded-lg bg-zinc-800 py-2 text-sm font-bold text-white hover:bg-zinc-700">+</button>
            </div>
          </div>
